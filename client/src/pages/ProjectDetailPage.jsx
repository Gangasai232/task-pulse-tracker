...
const BOARD_COLUMNS = [
  { id: 'BACKLOG', label: 'Backlog', borderTop: 'border-t-slate-500', countBg: 'bg-slate-800 text-slate-300' },
  { id: 'IN_PROGRESS', label: 'In Progress', borderTop: 'border-t-blue-500', countBg: 'bg-blue-950 text-blue-400 border border-blue-800/60' },
  { id: 'IN_REVIEW', label: 'In Review', borderTop: 'border-t-purple-500', countBg: 'bg-purple-950 text-purple-400 border border-purple-800/60' },
  { id: 'BLOCKED', label: 'Blocked', borderTop: 'border-t-rose-500', countBg: 'bg-rose-950 text-rose-400 border border-rose-800/60' },
  { id: 'DONE', label: 'Done', borderTop: 'border-t-emerald-500', countBg: 'bg-emerald-950 text-emerald-400 border border-emerald-800/60' },
];
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800/80 pb-6">
        <div className="space-y-1">
          <button
            ...
            className="text-xs font-semibold text-slate-400 hover:text-slate-200 flex items-center gap-1 mb-2 transition"
          >
            <ArrowLeft className="w-3.5 h-3.5" /> Back to Projects
          </button>
          <div className="flex items-center gap-3">
            <span className="px-3 py-1 rounded-xl bg-indigo-950/80 text-indigo-300 font-mono text-sm font-bold border border-indigo-800 shadow-md">
              {project.key}
            </span>
          ...
          <div className="flex items-center p-1 rounded-xl bg-slate-900 border border-slate-800 shadow-inner">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition ${
                viewMode === 'board' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 overflow-x-auto pb-4">
          {BOARD_COLUMNS.map((col) => {
            ...
            return (
              <div
                key={col.id}
                className={`glass-panel p-4 rounded-2xl border border-slate-800/80 border-t-2 ${col.borderTop} flex flex-col min-h-[520px] shadow-lg`}
              >
                {/* Column Header */}
                <div className="flex items-center justify-between pb-3 mb-3 border-b border-slate-800/80">
                  <div className="flex items-center gap-2">
                    <span className="font-display font-bold text-xs text-slate-200">{col.label}</span>
                    <span className={`h-5 px-2 rounded-full text-[10px] font-bold flex items-center justify-center ${col.countBg}`}>
                      {colTasks.length}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-3 flex-1 overflow-y-auto pr-1">
                  {colTasks.map((t) => (
                    <div
                      ...
                      className="p-4 rounded-xl glass-panel-hover bg-slate-900/70 border border-slate-800/90 cursor-pointer space-y-3 shadow-md"
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-mono text-[10px] font-bold text-indigo-400 px-1.5 py-0.5 rounded bg-indigo-950/60 border border-indigo-900/50">
                          {project.key}-{t.taskNum}
                        </span>
                        <PriorityBadge priority={t.priority} size="sm" />
                      </div>

                      <h4 className="text-xs font-semibold text-slate-100 line-clamp-2 leading-relaxed">{t.title}</h4>

                      {t.blockingTasks && t.blockingTasks.length > 0 && (
                        <div className="flex items-center gap-1 text-[10px] text-amber-400 font-medium bg-amber-950/50 px-2 py-0.5 rounded-md border border-amber-900/50 w-fit">
                            ...
                            <div className="flex -space-x-1.5">
                              {t.assignees.map((u) => (
                                <img
                                  ...
                                  src={u.avatarUrl}
                                  alt={u.name}
                                  className="w-4.5 h-4.5 rounded-full border-2 border-slate-950 object-cover"
                                />
                              ))}
                            </div>
                          ) : (
